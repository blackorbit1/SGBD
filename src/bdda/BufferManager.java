...
public class BufferManager {
	...
	private static final BufferManager bufferManager = new BufferManager();
	...
	/**
	 * Retourne un des buffers qui stockent le contenu d’une page dans une des
	 * cases
	 * 
	 * @param iPageId
	 *            l'ID de la page en question
	 *
	...
	public ByteBuffer getPage(PageId iPageId) {// vérifier si la page est déjà chargée
		...
		for (int i = 0; i < frames.size(); i++) {
			if (frames.get(i).getPageIdx() == iPageId.getPageIdx()) {
				...
				chaine += "///frame@pid=" + frames.get(i).getPageIdx() + "@d=" + dirty + "@pc="
						+ frames.get(i).getPin_count();
			}
		}
	 ...
	 *            l'ID de la page en question
	 * @param iIsDirty
	 *            TRUE pour changé FALSE pour inchangé
	 */
